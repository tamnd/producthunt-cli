...
describes what users get. Pick a format with `-o`, or let ph choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ph <command> -o table   # aligned columns for reading
...
```bash
ph <command> --fields id,title,url
```
...
```bash
ph <command>            # a table, because this is a terminal
ph <command> | wc -l    # JSONL, because this is a pipe
```
