...
Once `ph` is on your `PATH`, read the keyless feed. It needs no account and
works from anywhere:
...
The feed is the newest launches only. Topics, collections, users, comments, and
any filtered or ordered post list read the GraphQL API, which is reliable from
anywhere. Set free credentials and the rest of the commands light up:
...
The credentials come from Product Hunt's API dashboard and are read from the
environment only, never a flag. With nothing set, `ph` reads the web plane and
the feed-backed commands still work.

## Read one post

`post` takes the numeric id or the page slug and returns the full record:
tagline, vote and comment counts, the media, the topics, the makers, and the
edges into its comment thread and hunter.
...
Without credentials, `post` falls back to the feed: it answers for a recent
launch and reports not found for anything older.
...
The `ref` commands need no network. They turn any Product Hunt URL, path, Atom
id, `@username`, or bare id into a canonical id and back into an addressable URL:
